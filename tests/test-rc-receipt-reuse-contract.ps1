...
Require-Text $maintenance "6.0.3-native.19" "FORK-MAINTENANCE"
...
if (@($versions | Where-Object { $_ -ne "6.0.3-native.19" }).Count -ne 0) {
  $failures.Add("all .version-bump.json targets must equal immutable 6.0.3-native.19; got [$($versions -join ', ')]") | Out-Null
}
