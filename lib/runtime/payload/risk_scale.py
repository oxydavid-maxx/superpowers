...
def required_categories(risk):
    ...
    if risk.get("has_contract"):
        req.add("property")   # a state_data_contract-bearing cap MUST be struck by a
                              # property-based (Hypothesis) case over its invariant —
                              # scripted EP/BVA never exercise the unknown state space.
